...
import {
  MapPin, User, Phone, Home, Landmark, Hash, Navigation, MessageSquare, AlertCircle,
  CheckCircle, ShoppingCart, Plus, Minus, Star, Clock, Package, Bell,
  CreditCard, DollarSign, Wallet, Search
} from 'lucide-react';
...
const gmapsDir = (dLat, dLng) => `https://www.google.com/maps/dir/${ORIGIN.lat},${ORIGIN.lng}/${dLat},${dLng}`;
const wazeUrl  = (dLat, dLng) => `https://waze.com/ul?ll=${dLat},${dLng}&navigate=yes`;
  ...
  if (numberTrim) {
    ...
    if (sector && sector.trim()) {
      const url1b = `https://nominatim.openstreetmap.org/search?format=jsonv2&addressdetails=1&limit=8&countrycodes=cl&bounded=1&viewbox=${viewbox}&street=${encodeURIComponent(
        `${numberTrim} ${streetTrim}`
      )}&city=${encodeURIComponent(cityName)}&county=${encodeURIComponent(sector)}&country=Chile&dedupe=1&extratags=1`;
  ...
  for (const url of candidates) {
    try {
      ...
      const pick = (rec, precision, matchedNumber=false) => {
        ...
        if (isFinite(lat) && isFinite(lng)) return { lat, lng, precision, matchedNumber };
        return null;
      };
...
/* ===================== Tarjeta de Delivery ===================== */
const DeliveryOrderCard = ({ order, statusPillClass, orderStatuses, onDelivered, onConfirmPayment }) => {
    ...
    <div className="border-2 border-green-200 rounded-lg p-4 hover:shadow-md transition-shadow">
      ...
      {order.coordinates && (
        <div className="bg-gray-50 rounded-lg p-4 text-center">
          <p className="text-xs text-gray-500 mb-2">Inicio: <strong>{ORIGIN.name}</strong></p>
          <img
          ...
          />
          <button
            onClick={() => window.open(gmapsDir(order.coordinates.lat, order.coordinates.lng), '_blank')}
          ...
          <button
            onClick={() => window.open(wazeUrl(order.coordinates.lat, order.coordinates.lng), '_blank')}
...
const App = () => {
  ...
  useEffect(() => {
    ...
    (async () => {
      ...
      try {
        ...
        if (!cancel) { setGeocodedCoords(null); setSelectedCoords(null); }
      }
    })();
    return () => { cancel = true; };
  ...
  useEffect(() => {
    (async () => {
      ...
      const map = window.L.map(addrMapRef.current).setView([selectedCoords.lat, selectedCoords.lng], 17);
      window.L.tileLayer('https://{s}.tile.openstreetmap.org/{z}/{x}/{y}.png', { attribution: '© OpenStreetMap contributors' }).addTo(map);
      ...
      const marker = window.L.marker([selectedCoords.lat, selectedCoords.lng], { draggable: true }).addTo(map);
  ...
  const validateForm = () => {
    ...
    else if (customerData.name.trim().length < 2) e.name = 'El nombre debe tener al menos 2 caracteres';
    ...
    if (!selectedCoords) e.address = 'Ubique el pin en el mapa para confirmar la ubicación exacta';
  ...
  const selectCustomer = (c) => {
    ...
    if (c.coordinates) {
      ...
      setGeocodedCoords({ ...c.coordinates, precision: 'exact', matchedNumber: true });
    }
  ...
  const handleCreateOrder = async () => {
    ...
    setIsLoading(true);
    try {
      const addrStr = `${customerData.street} ${customerData.number}${customerData.sector ? `, ${customerData.sector}` : ''}`;
      ...
      setCustomers(prev => {
        const idx = prev.findIndex(c => (c.phone || '').replace(/\s|-/g,'') === (newOrder.phone || '').replace(/\s|-/g,''));
        const record = {
          name: newOrder.name,
      ...
      setCart([]);
      setCustomerData({
        name: '', phone: '', street: '', number: '', sector: '', city: 'Puerto Montt', references: '',
  ...
  const confirmPayment = (orderId) => {
    setOrders(prev => prev.map(o => o.id === orderId ? { ...o, paymentStatus: 'paid', paidAt: new Date().toISOString() } : o));
  };
  ...
  const getFilteredOrders = () => {
    switch (userRole) {
      case 'cook':     return orders.filter(o => ['pending', 'cooking', 'ready'].includes(o.status));
  ...
  const dashboard = useMemo(() => {
    ...
    const topClients = Object.values(orders.reduce((m,o)=>{
      const k = o.phone || o.name;
      m[k] = m[k] || { name:o.name, phone:o.phone, total:0, count:0 };
      m[k].total += o.total; m[k].count++;
      return m;
    }, {})).sort((a,b)=>b.total-a.total).slice(0,5);
    return { total, byStatus, due, deliveredCount: delivered.length, topClients };
  }, [orders]);
    ...
    <div className="min-h-screen bg-gradient-to-br from-red-50 to-orange-50">
      <div className="container mx-auto px-4 py-8">
        ...
        <div className="mb-4 bg-white rounded-lg shadow-lg p-4 flex flex-wrap items-center gap-2 justify-center">
          <button onClick={() => { setUserRole('cashier'); setShowDashboard(false); setActiveTab('promotions'); }}
                  className={`px-4 py-2 rounded-lg font-medium transition ${userRole==='cashier'&&!showDashboard ? 'bg-blue-500 text-white' : 'bg-gray-100 text-gray-700 hover:bg-gray-200'}`}>🏪 Cajero</button>
          <button onClick={() => { setUserRole('cook'); setShowDashboard(false); }}
                  className={`px-4 py-2 rounded-lg font-medium transition ${userRole==='cook'&&!showDashboard ? 'bg-orange-500 text-white' : 'bg-gray-100 text-gray-700 hover:bg-gray-200'}`}>👨‍🍳 Cocina</button>
          <button onClick={() => { setUserRole('delivery'); setShowDashboard(false); }}
                  className={`px-4 py-2 rounded-lg font-medium transition ${userRole==='delivery'&&!showDashboard ? 'bg-green-500 text-white' : 'bg-gray-100 text-gray-700 hover:bg-gray-200'}`}>🛵 Delivery</button>
          <span className="mx-2">|</span>
          <button onClick={() => setShowDashboard(true)}
                  className={`px-4 py-2 rounded-lg font-medium transition ${showDashboard ? 'bg-purple-600 text-white' : 'bg-gray-100 text-gray-700 hover:bg-gray-200'}`}>📊 Dashboard</button>
        </div>
          ...
          <div className="max-w-6xl mx-auto">
            <div className="grid grid-cols-1 md:grid-cols-4 gap-4 mb-6">
              ...
              <div className="bg-white rounded-lg p-4 shadow border">
                ...
                <p className="text-2xl font-bold text-gray-800">{(dashboard.byStatus.ready || 0)}</p>
              </div>
              <div className="bg-white rounded-lg p-4 shadow border">
                  ...
                  <tbody>
                    {dashboard.topClients.map((c,i)=>(
                      <tr key={i} className="border-t">
                        ...
                        <td className="py-2 font-semibold">${formatCLP(c.total)}</td>
                      </tr>
                    ))}
                    {dashboard.topClients.length===0 && (
                      <tr><td className="py-3 text-gray-500" colSpan={4}>Sin datos aún</td></tr>
                    )}
                  </tbody>
                </table>
              </div>
        ...
        {/* ===================== CAJERO – Promociones ===================== */}
        {!showDashboard && userRole === 'cashier' && activeTab === 'promotions' && (
        ...
        {!showDashboard && userRole === 'cashier' && activeTab === 'customer' && (
          <div className="max-w-5xl mx-auto">
            <div className="bg-white rounded-lg shadow-lg p-6">
              <div className="flex items-center gap-2 mb-4">
                ...
                <div className="ml-auto bg-gray-100 rounded-lg p-1 flex">
                  <button onClick={()=>setCustomerMode('new')}
                          className={`px-3 py-1 text-sm rounded ${customerMode==='new'?'bg-red-500 text-white':'text-gray-700'}`}>Nuevo</button>
                  <button onClick={()=>setCustomerMode('existing')}
                          className={`px-3 py-1 text-sm rounded ${customerMode==='existing'?'bg-red-500 text-white':'text-gray-700'}`}>Registrado</button>
                </div>
              </div>
              ...
              {customerMode === 'existing' && (
                <div className="mb-6 border rounded-lg p-4 bg-gray-50">
                  <label className="block text-sm font-medium text-gray-700 mb-2">
                    <Search size={16} className="inline mr-1" /> Buscar por nombre o teléfono
                  </label>
                  <input
                    type="text"
                    value={customerQuery}
                      ...
                      <div key={i} className="border rounded-lg p-3 bg-white">
                        <div className="flex justify-between">
                          <div>
                            ...
                            <p className="text-xs text-gray-500">{c.street} {c.number}{c.sector?`, ${c.sector}`:''}</p>
                          </div>
                          <button onClick={()=>selectCustomer(c)} className="h-8 px-3 rounded bg-blue-500 hover:bg-blue-600 text-white text-sm">Usar</button>
                        </div>
                      </div>
                    ))}
                    {filteredCustomers.length===0 && (
                      <p className="text-sm text-gray-500">Sin resultados. Cambia el criterio o crea cliente nuevo.</p>
                    )}
                  </div>
                </div>
              )}
              ...
              <div className="grid grid-cols-1 md:grid-cols-2 gap-4">
                <div>
                  <label className="block text-sm font-medium text-gray-700 mb-1"><User size={16} className="inline mr-1" /> Nombre</label>
                  <input
                    type="text"
                ...
                <div>
                  <label className="block text-sm font-medium text-gray-700 mb-1"><Landmark size={16} className="inline mr-1" /> Calle</label>
                  <input
                    type="text"
                    value={customerData.street}
                    onChange={(e) => setCustomerData(v => ({ ...v, street: e.target.value }))}
                    className={`w-full px-3 py-2 border rounded-lg focus:ring-2 focus:ring-red-500 focus:border-transparent ${errors.street ? 'border-red-500' : 'border-gray-300'}`}
                ...
                <div>
                  <label className="block text-sm font-medium text-gray-700 mb-1"><Hash size={16} className="inline mr-1" /> Número</label>
                  <input
                    type="text"
                    value={customerData.number}
                    onChange={(e) => setCustomerData(v => ({ ...v, number: e.target.value }))}
                    className={`w-full px-3 py-2 border rounded-lg focus:ring-2 focus:ring-red-500 focus:border-transparent ${errors.number ? 'border-red-500' : 'border-gray-300'}`}
                    placeholder="Ej: 6191"
                  />
                ...
                <div>
                  <label className="block text-sm font-medium text-gray-700 mb-1"><Home size={16} className="inline mr-1" /> Población / Sector (opcional)</label>
                  <input
                    type="text"
                    value={customerData.sector}
                    onChange={(e) => setCustomerData(v => ({ ...v, sector: e.target.value }))}
                    className="w-full px-3 py-2 border rounded-lg focus:ring-2 focus:ring-red-500 focus:border-transparent border-gray-300"
              ...
              <div className="mt-4 grid grid-cols-1 md:grid-cols-3 gap-4">
                <div>
                  <label className="block text-sm font-medium text-gray-700 mb-1 flex items-center gap-1"><CreditCard size={16}/> Método de pago</label>
                  <select
                    value={customerData.paymentMethod}
                    onChange={(e)=>setCustomerData(v=>({...v, paymentMethod: e.target.value }))}
                    className="w-full px-3 py-2 border border-gray-300 rounded-lg focus:ring-2 focus:ring-red-500 bg-white capitalize"
                  >
                    {PAYMENT_METHODS.map(m=><option key={m.value} value={m.value} className="capitalize">{m.label}</option>)}
                  </select>
                </div>
                <div>
                  <label className="block text-sm font-medium text-gray-700 mb-1 flex items-center gap-1"><DollarSign size={16}/> Estado</label>
                  <select
                    value={customerData.paymentStatus}
                    onChange={(e)=>setCustomerData(v=>({...v, paymentStatus: e.target.value }))}
                    className="w-full px-3 py-2 border border-gray-300 rounded-lg focus:ring-2 focus:ring-red-500 bg-white"
                  >
                ...
                {customerData.paymentStatus==='due' && (
                  <div>
                    <label className="block text-sm font-medium text-gray-700 mb-1 flex items-center gap-1"><Wallet size={16}/> Medio al recibir</label>
                    <select
                      value={customerData.dueMethod}
                      onChange={(e)=>setCustomerData(v=>({...v, dueMethod: e.target.value }))}
                      className="w-full px-3 py-2 border border-gray-300 rounded-lg focus:ring-2 focus:ring-red-500 bg-white capitalize"
                    >
              ...
              {(geocodedCoords || selectedCoords) && (
                <div className="mt-4 bg-white border rounded-lg p-3">
                  <div className="flex items-center justify-between">
                    <h4 className="font-semibold text-gray-700 mb-2">Ubicación exacta (mueve el pin si es necesario)</h4>
                    {customerData.number && geocodedCoords && geocodedCoords.precision !== 'exact' && (
                  ...
                  <div className="text-xs text-gray-600 mt-2">
                    Origen: <b>{ORIGIN.name}</b>
                    {selectedCoords && (
                      <> — Selección: <b>{selectedCoords.lat.toFixed(6)}, {selectedCoords.lng.toFixed(6)}</b></>
                    )}
                  </div>
                </div>
              )}
              ...
              {cart.length > 0 && (
                <div className="mt-4 bg-green-50 border border-green-200 rounded-lg p-4">
                  ...
                  <div className="border-t border-green-300 mt-2 pt-2">
                    ...
                    <p className="text-sm text-green-600">Tiempo estimado de cocina: {getEstimatedCookingTime()} min</p>
                  </div>
                </div>
              )}
        ...
        {/* ===================== CAJERO – Carrito ===================== */}
        {!showDashboard && userRole === 'cashier' && activeTab === 'cart' && cart.length > 0 && (
            ...
            <div className="bg-white rounded-lg shadow-lg p-6">
              ...
              <div className="border-t border-gray-200 pt-4 mt-6">
                ...
                <div className="text-center space-y-2">
                  <div className="text-sm text-gray-600 flex items-center justify-center gap-2"><Clock size={16} />Tiempo estimado de preparación: {getEstimatedCookingTime()} min</div>
          ...
          <div className="max-w-6xl mx-auto">
            <div className="bg-white rounded-lg shadow-lg p-6">
              ...
              <div className="grid grid-cols-1 lg:grid-cols-2 gap-6">
                {getFilteredOrders().filter(o=>['pending','cooking','ready'].includes(o.status)).length === 0 ? (
                  <div className="col-span-2 text-center py-8 text-gray-500">
                    <Package size={48} className="mx-auto mb-4 text-gray-300" />
                    <p>No hay pedidos en este momento</p>
                  </div>
                ) : (
                  getFilteredOrders().filter(o=>['pending','cooking','ready'].includes(o.status)).map(order => {
                    const status = orderStatuses[order.status];
                    const StatusIcon = status.icon;

                    let packingText = null;
                    if (order.status === 'ready' && order.packUntil) {
                      const secs = Math.max(0, Math.ceil((order.packUntil - Date.now())/1000));
                      ...
                      <div key={order.id} className="border-2 border-gray-200 rounded-lg p-4 hover:shadow-md transition-shadow">
                        ...
                        <div className="mb-4">
                          ...
                          <div className="mt-2 pt-2 border-t border-gray-200">
                            <p className="text-sm text-orange-600 font-medium">⏱️ Máximo estimado: {order.estimatedTime} min</p>
                            {packingText && (
                              <p className={`text-sm font-semibold mt-1 ${packingText.includes('finalizado')?'text-green-600':'text-amber-700'}`}>
                                {packingText}
                              </p>
                            )}
                          </div>
            ...
            <div className="bg-white rounded-lg shadow-lg p-6">
              ...
              <div className="grid grid-cols-1 lg:grid-cols-2 gap-6">
                {getFilteredOrders().filter(o=>o.status==='ready').length === 0 ? (
                  ...
                  getFilteredOrders().filter(o=>o.status==='ready').map(order => (
                    <DeliveryOrderCard
                      key={order.id}
                      order={order}
            ...
            <div className="bg-white rounded-lg shadow-lg p-6">
              ...
              <div className="grid grid-cols-1 lg:grid-cols-2 gap-6">
                {getFilteredOrders().filter(o=>o.status==='delivered').length === 0 ? (
                  ...
                  getFilteredOrders().filter(o=>o.status==='delivered').map(order => (
                    <div key={order.id} className="border rounded-lg p-4">
                      <div className="flex items-center justify-between">
                        <div>
                          <p className="font-semibold text-gray-800">#{order.id.toString().slice(-4)} — {order.name}</p>
                          <p className="text-xs text-gray-500">{order.address}</p>
                        </div>
                        <span className="px-2 py-0.5 rounded text-xs bg-blue-100 text-blue-700">Entregado</span>
                      </div>
                      <div className="mt-2 text-sm">
                        <span className="text-gray-700">Pago: </span>
                        <span className="font-medium">{order.paymentStatus==='paid' ? 'Pagado' : 'Por pagar'}</span>
                        {order.paymentStatus==='paid' && order.paidAt && (
                          <span className="ml-2 text-xs text-gray-500">({new Date(order.paidAt).toLocaleString('es-CL')})</span>
                        )}
                      </div>
                    </div>
                  ))
                  ...
                  <div key={order.id} className="border border-gray-200 rounded-lg p-4">
                    <div className="grid grid-cols-1 lg:grid-cols-5 gap-4">
                      ...
                      <div>
                        ...
                        {order.geocodePrecision && order.geocodePrecision!=='exact' && (
                          <p className="text-xs text-amber-700 mt-1">Punto aprox. ({order.geocodePrecision})</p>
                        )}
                      </div>
                      <div>
                      ...
                      <div>
                        ...
                        <div className="mt-1 text-xs">
                          <span className={`px-2 py-0.5 rounded ${order.paymentStatus==='paid'?'bg-green-100 text-green-700':'bg-red-100 text-red-700'}`}>
                      ...
                      <div className="text-center">
                        <img
                          src={`https://api.qrserver.com/v1/create-qr-code/?size=80x80&data=${encodeURIComponent(order.wazeUrl || wazeUrl(order.coordinates?.lat ?? 0, order.coordinates?.lng ?? 0))}`}
                          alt="QR"
